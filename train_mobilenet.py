...
def model_load(IMG_SHAPE=(224, 224, 3), class_num=245):
    ...
    base_model = tf.keras.applications.MobileNetV2(input_shape=IMG_SHAPE,
                                                   include_top=False,
                                                   weights=None)  # 加载mobilenetv2模型

    weights_path ='F:\\mobilenet_v2_weights_tf_dim_ordering_tf_kernels_1.0_224_no_top.h5'
    base_model.load_weights(weights_path)  # 从文件中加载权重
...
def train(epochs):
    ...
    train_ds, val_ds, class_names = data_load("F://trash_jpg", 224, 224, 80)  # 记录数据集位置
    ...
    model.save("models/mobilenet.h5")  #模型保存位置，保存模型
